...
5. Wait for deploy (~3-5 min) → open `https://enrichly-api-qhj2.onrender.com/api/health`
...
5. **Environment Variable:** `NEXT_PUBLIC_API_URL` = `https://enrichly-api-qhj2.onrender.com`
   (this is baked at build time — no trailing slash)
6. Click **Deploy**
7. Open `https://enrichly-one.vercel.app` → log in with `demo@enrichly.dev / password123`
...
7. **Health**: Visit `https://enrichly-api-qhj2.onrender.com/api/health` → `{"status":"healthy"}`
...
6. **Build arg:** `NEXT_PUBLIC_API_URL=https://enrichly-api-qhj2.onrender.com`
7. **Plan:** Free
...
| `FRONTEND_URL` | API (Render) | `https://enrichly-one.vercel.app` |
