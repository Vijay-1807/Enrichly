    ...
    <div className="mx-auto grid max-w-4xl items-stretch gap-5">
      <div className="card-dark animate-enter hidden flex-col justify-between p-8 sm:flex" style={{ ['--d' as any]: '0ms' }}>
        <span className="flex h-10 w-10 items-center justify-center rounded-lg bg-white/10">
          <LineIcon d={ICONS.bolt} className="h-5 w-5 text-emerald-200" />
        </span>
        <div>
          <p className="h-display text-[34px] text-white">Your workers<br /><span className="text-white/50">are waiting.</span></p>
          <p className="mt-3 max-w-xs text-[15px] leading-relaxed text-white/60">Create an account and run your first job in under a minute.</p>
        </div>
        <p className="eyebrow !text-white/40">enrichly · job automation</p>
      </div>
      <div className="animate-enter" style={{ ['--d' as any]: '80ms' }}>
        <h1 className="h-display text-4xl">Create account</h1>
        <p className="mt-2 text-[15px] text-stone-500">Free, instant, no setup needed.</p>
        <form onSubmit={submit} className="card mt-6 space-y-4 p-5 sm:p-6">
          ...
          <button className="btn-primary w-full py-3" disabled={busy}>{busy ? 'Creating…' : <>Register <LineIcon d={ICONS.arrow} className="h-4 w-4" /></>}</button>
          <p className="text-center text-sm text-stone-500">Have an account? <Link href="/login" className="font-semibold text-[#1d4a38] underline underline-offset-4">Sign in</Link></p>
        </form>
      </div>
    </div>
  );
