...
- Name: full-flow.spec.ts >> Full Job Automation Platform - Requirements Verification >> Authentication & Authorization >> Register new user
- Location: e2e\full-flow.spec.ts:29:9
...
- generic [active] [ref=f1e1]:
  ...
  - main [ref=f1e35]:
    - generic [ref=f1e36]:
      - generic [ref=f1e37]:
        - generic [ref=f1e41]:
          - paragraph [ref=f1e42]: Your workersare waiting.
          - paragraph [ref=f1e43]: Create an account and run your first job in under a minute.
        - paragraph [ref=f1e44]: enrichly · job automation
      - generic [ref=f1e45]:
        - heading "Create account" [level=1] [ref=f1e46]
        - paragraph [ref=f1e47]: Free, instant, no setup needed.
        - generic [ref=f1e48]:
          - generic [ref=f1e49]:
            - generic [ref=f1e50]: Email
            - textbox [ref=f1e51]
          - generic [ref=f1e52]:
            - generic [ref=f1e53]: Password · min 8 chars
            - textbox [ref=f1e54]
          - button "Register" [ref=f1e55] [cursor=pointer]
          - paragraph [ref=f1e58]:
...
> 32  |       await page.fill('input[type="email"]', email);
  ...
  109 |     });
  110 | 
  111 |     test('Create interval job', async ({ page }) => {
  112 |       await page.goto('/jobs/new');
  113 |       await page.fill('input[placeholder="Sync users every hour"]', 'Interval Job');
  114 |       await page.fill('input[placeholder="https://api.example.com/webhook"]', 'http://api:8080/api/demo/echo');
  115 |       await page.selectOption('select >> nth=0', 'Interval');
  116 |       await page.fill('input[type="number"] >> nth=0', '300');
  117 |       await page.click('button:has-text("Create job")');
  118 |       await expect(page.locator('text=every 300s')).toBeVisible();
  119 |     });
  120 |   });
  121 | 
  122 |   test.describe('Run Now & Execution History', () => {
  123 |     test.beforeEach(async ({ page }) => {
  124 |       await login(page);
  125 |     });
  126 | 
  127 |     test('Run now creates execution', async ({ page }) => {
  128 |       await page.goto('/jobs/new');
  129 |       await page.fill('input[placeholder="Sync users every hour"]', 'Run Test Job');
  130 |       await page.fill('input[placeholder="https://api.example.com/webhook"]', 'http://api:8080/api/demo/echo');
  131 |       await page.click('button:has-text("Create job")');
  132 |       await page.waitForURL(/\/jobs\/.*/);
```